...
```text
testvideo/01_复旦山哥_半泽直树与银行风险_10min.mp4
```
...
实现及自动测试通过后，在当前 macOS 开发机上使用 `testvideo/01_复旦山哥_半泽直树与银行风险_10min.mp4` 执行最终端到端验证：
...
5. 校验 JSON 中媒体时长与 632.169 秒的误差不超过一秒，所有 segment 时间戳单调且处于媒体范围内。
